...
for SEED in 2021 2022 2023; do
  for PRED_LEN in 96 192 336 720; do
    run_one "Weather_pl${PRED_LEN}_s${SEED}" \
      ...
      --model_id "Weather_${PRED_LEN}_s${SEED}"
  done
  echo "Weather seed=${SEED} done"
done

echo "Weather multi-seed complete"
if [ -d "/content/drive/MyDrive" ]; then
  cp "${SCRIPT_DIR}/result.txt" \
     "/content/drive/MyDrive/PatchLinear_seeds_weather_$(date +%Y%m%d_%H%M%S).txt"
