#!/usr/bin/env bash
# Multi-seed runs — Exchange (updated lr=0.0001)
set -euo pipefail
...
for SEED in 2021 2022 2023; do
  for PRED_LEN in 96 192 336 720; do
    run_one "Exchange_pl${PRED_LEN}_s${SEED}" \
  ...
  echo "Exchange seed=${SEED} done"
done

echo "Exchange multi-seed complete"
if [ -d "/content/drive/MyDrive" ]; then
  cp "${SCRIPT_DIR}/result.txt" \
     "/content/drive/MyDrive/PatchLinear_seeds_exchange_$(date +%Y%m%d_%H%M%S).txt"
